...
- **Paginated** [blog](/2/), [tag](https://log1x.com/tag/dummy/), and [author](/author/gridsome/) archives
...
```js
{
  ...
  options: {
    ...
    feedOptions: {
      ...
      feed_url: 'https://log1x.com/feed.xml',  // <- update, leave the file name
      site_url: 'https://log1x.com'  // <- update
    },
    feedItemOptions: node => ({
      title: node.title,
      description: node.description,
      url: 'https://log1x.com/' + node.slug,  // <- update
      author: node.author,
      date: node.date
    }),
    output: {
...
<div class="p-4 mb-6 leading-normal text-orange-900 bg-orange-100 border-l-4 border-orange-500 md:mx-6" role="alert">
...
The `color` prop can be any color name from your `tailwind.js` config. If you omit it, the alert will use <span class="inline-block px-2 py-px border-l-4 bg-blue-lightest border-blue text-blue-darker stext-sm">blue</span> as a fallback.
